...
if pet_datasets:
    ...
    if pet_volume is not None:
        ...
        pvh_table_absolute, pvh_table_relative = export_cumulative_pet_histogram(
            pet_volume, structure_masks, structures, voxel_volume_ml, "dvh_data_combined"
        )
        ...
        num_structures = len([k for k in pvh_table_absolute.keys() if k != 'PET_Bq_per_mL'])
        ...
        pet_values = pvh_table_absolute["PET_Bq_per_mL"]
        structure_index = 0
        for struct_name, volumes in pvh_table_absolute.items():
            if struct_name == "PET_Bq_per_mL":
                continue
        ...
        abs_plot_path = os.path.join("dvh_data_combined", "PVH_Absolute_Plot.png")
        ...
        for struct_name, volumes in pvh_table_relative.items():
            ...
            plt.plot(pvh_table_relative["PET_Bq_per_mL"], volumes, 
                    label=clean_name, 
                    linewidth=2,
                    color=colors[structure_index % len(colors)])
            structure_index += 1
        ...
        rel_plot_path = os.path.join("dvh_data_combined", "PVH_Relative_Plot.png")
        ...
        for struct_name, volumes in pvh_table_absolute.items():
            ...
            rel_struct_name = struct_name.replace("_Volume_mL", "_RelativeCumVolume")
            ...
            for h in headers:
                if h == "Dose_Gy":
                    ...
                    if i == 0:
                        ...
                        delta = dvh_table_relative[h][i] - dvh_table_relative[h][i - 1]
                    row.append("{:.6f}".format(abs(delta)))
            writer.writerow(row)
