            ...
            with open(summary_csv, mode='w', newline='') as summary_file:
                ...
                for sid, struct in structures.items():
                    try:
                        ...
                        dose_origin = np.array(rd_dataset.ImagePositionPatient)
                        dose_spacing = np.array(list(rd_dataset.PixelSpacing) + [rd_dataset.SliceThickness])
                        dose_direction = np.eye(3) # if RTDOSE has a direction
                        
                        stats = compute_dvh_from_mask(
                            struct['name'], contours_by_slice, dose_array, voxel_volume, global_bins,
                            dose_origin, dose_spacing, dose_direction
                        )
                        ...
                        if stats:
                            ...
                            rel_dose = (np.array(stats['bins']) / prescription_dose) * 100
                            rel_volume = (np.array(stats['cumulative']) / stats['volume']) * 100
                            ...
                            with open(dvh_path, mode='w', newline='') as dvh_file:
                                ...
                                for d, v in zip(abs_dose, abs_volume):
                                    dvh_writer.writerow([f"{d:.4f}", f"{v:.7f}"])
                            ...
                            with open(dvh_bqml_path, mode='w', newline='') as bqml_file:
                                ...
                                for d_bqml, v in zip(abs_dose_bqml, abs_volume):
                                    bqml_writer.writerow([f"{d_bqml:.4f}", f"{v:.2f}"])
