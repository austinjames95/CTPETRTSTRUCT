...
def export_cumulative_pet_histogram(pet_volume, structure_masks, structures, voxel_volume_ml, output_path):
    # Define bins (adjust bin width if needed)
    bin_count = 100
    pet_min = np.min(pet_volume)
    pet_max = np.max(pet_volume)
    bins = np.linspace(pet_min, pet_max, bin_count + 1)
    bin_centers = 0.5 * (bins[:-1] + bins[1:])

    # Initialize cumulative table
    pvh_table = {"PET_Bq_per_mL": bin_centers.tolist()}

    for sid, mask in structure_masks.items():
        values = pet_volume[mask]
        if values.size == 0:
            continue

        # Compute cumulative volume: ≥ each bin threshold
        structure_volumes = []
        for threshold in bins[:-1]:
            count = np.sum(values >= threshold)
            volume = count * voxel_volume_ml
            structure_volumes.append(volume)

        safe_name = structures[sid]['name'].replace(" ", "_").replace("/", "_").replace("-", "_")
        pvh_table[f"{safe_name}_Volume_mL"] = structure_volumes

    # Write to CSV
    with open(output_path, 'w', newline='') as f:
        writer = csv.writer(f)

        writer.writerow(["# Cumulative PET Volume Histogram (PVH)"])
        writer.writerow(["# PET bin unit: Bq/mL | Volume unit: mL"])
        writer.writerow([])

        headers = list(pvh_table.keys())
        writer.writerow(headers)

        for i in range(len(bin_centers)):
            row = [f"{pvh_table[h][i]:.6f}" for h in headers]
            writer.writerow(row)

    print(f"\n PET Volume Histogram written to: {output_path}")
