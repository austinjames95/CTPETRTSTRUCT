...
import csv
from viewer import visualize_pet_data, create_pet_volume
...
if pet_datasets:
    ...
    if rd_dataset:
        ...
        if pet_volume is not None:
            ...
            zoom_factors = (
                target_shape[0] / pet_volume.shape[0],
                target_shape[1] / pet_volume.shape[1], 
                target_shape[2] / pet_volume.shape[2]
            )
            
            print(f"Applying zoom factors: {zoom_factors}")
            pet_volume = ndimage.zoom(pet_volume, zoom_factors, order=1)
            print(f"Aligned PET shape: {pet_volume.shape}")
    else:
        # No dose data - create PET normally
        pet_volume, sorted_pet = create_pet_volume(pet_datasets)
    
    if pet_volume is not None:
        # Wait to get structures before visualizing
        structures = None
        if rs_dataset:
            try:
        ...
        # Call enhanced visualization with CSV export
        ...
        # Enhanced PET summary with proper units
